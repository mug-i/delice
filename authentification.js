...
import { getUtilisateurById,getUtilisateurByIdentifiant } from "./model/utilisateur.js";
...
passport.use(new Strategy(config, async (courriel, motDePasse, done) => {
    ...
    try {
        ...
        // courriel ici
        const utilisateur = await getUtilisateurByIdentifiant(courriel);

        // Si on ne trouve pas l'utilisateur, on
        // retourne que l'authentification a échoué
        // avec un message
        if (!utilisateur) {
            return done(null, false, { erreur: 'mauvais_identifiant' });
        }
